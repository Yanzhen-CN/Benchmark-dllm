...
def _select_hellobench(
    ...
    for profile, count in zip(profiles, counts):
        ...
        selected.extend(
            _with_meta(sample, max_new_tokens=max_new_tokens) for sample in chosen
        )
    return selected
...
def _select_ruler(
    ...
    per_group = _positive_int(
        config.get("samples_per_task_per_context_window"),
        "samples_per_task_per_context_window",
    )
    position_counts = _balanced_counts(per_group, len(positions))
        ...
        for task_type in task_types:
            for position, count in zip(positions, position_counts):
                ...
                chosen = _take(
                    ...
                    count,
                    rng,
                    f"RULER task={task_type!r} position={position!r} window={window}",
                )
                selected.extend(
